...
zygotine.M.SEGInformedVarModelParameters = function (logN, oel) {
    ...
    if (!logN) {
        this.muLower = 40;
        this.muUpper = 125;
        this.logSigmaMu = 1.0986122886681098; //#(GM = 3) // corrig� le 31 mai log(3)
...
* @constructor
* @param {zygotine.M.SEGInformedVarModel} model - le mod�le qui produira les r�sultats seront affich�s
*
...
* @param {zygotine.M.SEGInformedVarModelParameters} specificParameters  - les param�tres propres au mod�le
...
/**
* m�thode priv�e, ne devrait �tre appel�e que de doParameterValidation ou de compute.
* les deux param�tres doivent �tre pr�sents et bien d�finits. Le premier est un entier et le second un bool�en.
* @method
*
...
zygotine.M.SEGInformedVarModel.prototype.run = function (prngSeed, validationOnly) {
    ...
    this.validateParameters__(result); // M�thode de BaseModel
    ...
    // L'objectif n'�tait pas que de valider les param�tres.
    // De plus les param�tres sont valides.
...
zygotine.M.SEGUninformativeModelParameters = function (logN, oel) {
    zygotine.M.ModelParameters.call(this, logN, oel);
    if (logN) {
        // this.logN = logN; // modif f�vrier 2020 instruction inutile
        this.muLower = -20;
        this.muUpper = 20;
        this.initMu = -1.2039728043259361; // log(0.3) 
...
* @constructor
* @param {zygotine.M.SEGUninformativeModel} model - le mod�le auquel sont rattach�s les r�sultats
*
* @property {string} mcmcParameters
* @property {string} measureList
* @property {zygotine.M.Messages} messages
* @property {zygotine.M.SEGUninformativeModel} model // modif f�vrier 2020 : le type du mod�le �tait incorrect.
* @property {zygotine.M.PastDataSummary} pastData 
* @property {string} specificParameters
* @property {number} totalNumberOfIterations
*/
...
* @param {zygotine.M.SEGInformedVarModelParameters} specificParameters - des param�tres propres au mod�le en balisant l'ex�cution
* @param {zygotine.M.McmcParameters} mcmcParameters - les param�tres MCMC 
*
* @property {string} className - le nom de la classe, h�rit� de BaseModel
* @property {boolean} hasError - vrai si une erreur a �t� d�tect�, sinon faux, h�rit� de LoggingError via BaseModel
* @property {zygotine.M.McmcParameters} mcmcParameters = les param�tres MCMC, h�rit�s de BaseModel
* @property {zygotine.M.MeasureList} measureList - la liste de mesures utilis�e, h�rit�e de BaseModel
* @property {zygotine.M.Messages} messages - une liste de messages d'erreurs ou de mises en garde, h�rit�s de LoggingError via BaseModel
* @property {zygotine.M.SEGInformedVarModelParameters} specificParameters  - les param�tres propres � l'ex�cution du mod�le, h�rit�s de BaseModel
*/
...
// modif f�vrier 2020 ajout du commentaire qui suit
/**
* m�thode priv�e, ne devrait �tre appel�e que de doParameterValidation ou de compute.
* les deux param�tres doivent �tre pr�sents et bien d�finis. Le premier est un entier et le second un bool�en.
* @method
*
...
zygotine.M.SEGUninformativeModel.prototype.run = function (prngSeed, validationOnly) {
    ...
    var result = new zygotine.M.SEGUninformativeModelResult(this); // modif f�vrier 2020 : on a remplac� SEGUninformativeModelParameters par SEGUninformativeModelResult
    this.validateParameters__(result); // M�thode de BaseModel
    ...
    //les param�tres d'entr�e sont valides. On sauvegarde la racine du g�n�rateur de nbr al�atoires.

    // modif f�vrier 2020, 3 lignes suivantes mise en commentaires, la responsabilit� des seed rel�ve de la m�thode compute.
    /*
    ...
    */
    ...
    var
        ...
        tauCondAlpha = (data.n - 1.0) / 2.0, // modif f�vrier 2020: on a remplac� "data.n / 2 - 1" par "(data.n - 1.0) / 2.0"
...
* @constructor
* @param {boolean} logNormDstrn - lorsque faux, on consid�re faire affaire avec la loi normale.
* @param {boolean} uupOnSds  - lorsque vrai, on utilise la loi normale pour les priors des �carts-type
...
* @property {number} muOverallLower - borne inf�rieure pour la prior 'uniforme' de muOverall
* @property {number} muOverallUppper - borne sup�rieure pour la prior 'uniforme' de muOverall
* @property {number[]} sigmaBetweenRange - lorsque uupOnSds est vrai, d�finit l'interval balisant la prior 'uniforme' de sigmaBetween
* @property {number[]} sigmaWithinRange - lorsque uupOnSds est vrai, d�finit l'interval balisant la prior 'uniforme' de sigmaWithin
* @property {number} logSigmaWithinMu - lorsque uupOnSds est faux, param�tre pour la prior de sigmaWithin
* @property {number} logSigmaWithinPrec - lorsque uupOnSds est faux, param�tre pour la prior de sigmaWithin
* @property {number} logsigmaBetweenMu - lorsque uupOnSds est faux, param�tre pour la prior de sigmaBetween
* @property {number} logsigmaBetweenPrec - lorsque uupOnSds est faux, param�tre pour la prior de sigmaBetween
...
zygotine.M.BetweenWorkerModelParameters = function (logN, uupOnSds, oel) {
    ...
    //les 2 premiers param�tres, balisant muOverall, sont pr�sents quelle que soit la valeur de uupOnSds
    ...
    // les 6 param�tres suivants sont divis�s en 2 groupes selon la valeur de uupOnSds
    if (uupOnSds) {
        if (logN) {
...
* @constructor
* @param {zygotine.M.MeasureList} measureList - une liste de mesures
* @param {zygotine.M.BetweenWorkerModelParameters} specificParameters - des param�tres propres au mod�le qui en balisent l'ex�cution
* @param {zygotine.M.McmcParameters} mcmcParameters - les param�tres MCMC 
*
* @property {string} className - le nom de la classe, h�rit� de BaseModel
* @property {boolean} hasError - vrai si une erreur a �t� d�tect�, sinon faux, h�rit� de LoggingError via BaseModel
* @property {zygotine.M.McmcParameters} mcmcParameters = les param�tres MCMC, h�rit�s de BaseModel
* @property {zygotine.M.MeasureList} measureList - la liste de mesures utilis�e, h�rit�e de BaseModel
* @property {zygotine.M.Messages} messages - une liste de messages d'erreurs, de messages d'information ou de mises en garde, h�rit�e de LoggingError via BaseModel
* @property {zygotine.M.BetweenWorkerModelParameters} specificParameters  - les param�tres propres � l'ex�cution du mod�le, h�rit�s de BaseModel
*/
...
/* modif f�vrier 2020. m�thode calcSigma : nouvelle version plus bas
...
// modif f�vrier 2020. nouvelle version de la m�thode.
zygotine.M.BetweenWorkerModel.prototype.calcSigma = function (n, sigmaRange, u) {
    if (typeof u === 'undefined') {
        u = zygotine.S.uniform.sample(1, 0, 1);
    }
...
zygotine.M.BetweenWorkerModel.prototype.run = function (prngSeed, validationOnly) {
    ...
    result.workerChainLabelPrefixes = workerDigest.workerIds.map(function (w) { return "mu_" + w; }); // workerDigest.workerIds est ordonn� alphab�tiquement
    ...
    var
        ...
        sigmaBetween;
    // Suite a une demande, 
    // on utilise pour sigmaWithin et muOverall, plut�t que le r�sultat d'un calcul dans workerDigest, 
    // les valeurs de initMuOverall et initSigmaWithin incluses dans les param�tres(specificParameters).
    // Les param�tres initMuOverall et initSigmaWithin peuvent �tre ajust�es par l'usager a travers le UI.
    ...
    // En autant qu'il y ait des donn�es censur�es, on mettra � jour ces infos � chaque it�ration
    // sinon les valeurs calcul�es ci-bas demeureront inchang�es.
    var
    ...
    // Rapplelons que logSigmaWithinSd et logSigmaBetweenSd ne seront utilis�s que si uupOnSds est faux!
    var
    ...
    // 2 variables qui ne seront utilis�es ssi uupOnSds est vrai, autremenent elles demeureront de type undefined.
    ...
    if (uupOnSds) {
        ...
        tauBetweenRange = [1.0 / (sp.sigmaBetweenRange[1] * sp.sigmaBetweenRange[1]), 1.0 / (sp.sigmaBetweenRange[0] * sp.sigmaBetweenRange[0])]; // il se peut que  sp.sigmaWihtinRange soit 0, alors la borne sup�rieure sera Infinity.
    }
    ...
    for (iter = 0; iter < nIterations; iter++) {
        ...
        if (data.anyCensored) {
            ...
            //On prend les moyennes qui varient en compte les valeurs g�n�r�es associ�es aux mesures censur�es
            averages = workerDigest.getAverages();
        }
        ...
        if (uupOnSds) {
            if (beta < this.betaMin) {
                ...
                let tau = zygotine.O.rGammaTruncated((data.n - 1.0) / 2.0, beta, tauWithinRange); // modif f�vrier 2020 : on a remplac� "(data.n / 2) - 1" "(data.n - 1.0) / 2.0"
                //let tau = zygotine.O.rGammaTruncated((data.n / 2.0) - 1.0, beta, tauWithinRange); // modif f�vrier 2020: mettre en commentaire
        ...
        if (uupOnSds) {
            if (beta < this.betaMin) {
                ...
                let tau = zygotine.O.rGammaTruncated((workerDigest.nWorkers - 1.0) / 2.0, beta, tauBetweenRange); // modif f�vrier 2020 : on a remplac� "(data.nWorkers / 2) - 1" "(data.nWorkers - 1.0) / 2.0"
                //let tau = zygotine.O.rGammaTruncated((workerDigest.nWorkers / 2.0) - 1.0, beta, tauBetweenRange) // modif f�vrier 2020: mettre en commentaire
...
/**
* Repr�sente les param�tres de d�part du mod�le BetweenWorkerModel
* @constructor
* @property {boolean} logN - un bool�en, vrai pour la loi log-normale et faux pour la loi normale
...
* @property {number} muOverallLower - param�tre commun au 2 types de priors possibles (uupOnSds)
* @property {number} muOverallUpper - param�tre commun au 2 types de priors possibles (uupOnSds)
...
* @property {boolean} uupOnSds -  un bool�en, vrai si l'on utilise une prior uniforme, faux sinon ...
*/
...
zygotine.M.Worker = function (id) {
    this._mean = NaN; // champ priv�, facilite le calcul de mu (dans getInits du WorkerDigest), sera mis � NaN lorsque devenu inutile.
    this.mu = NaN;
};
...
zygotine.M.WorkerDigest = function (measureArray) {
    ...
    this._measureArray = []; // selon l'ordre de pr�sentation, ie celui de la chaine de caract�re ayant servi � d�finir la MeasureList, ordre que pr�serve measureArray ...
    this._measureByType = { uncensored: [], greaterThan: [], lessThan: [], interval: [] };
    this._measureByWorker = [];
    //    this.workerInfo = {};
    this._measureOrderedAsR = []; // l'ordre de R, ie "non-censur�" suivi de  ">" suivi de  "<" suivi de [ , ]! Dans chaque groupe l'ordre est donn� par celui de la MeasureList
    var m;
    var o;
    ...
    for (let i = 0; i < measureArray.length; i++) {
        ...
        this._measureArray.push(extM); // l'ordre est celui du param�tre measureArray, donc l'ordre original des mesures.
        this._measureByType[extM.type].push(extM); // pour un type donn�, l'ordre est celui du parametre measureArray.
    ...
    this.workerIds = Object.keys(this._measureByWorker).sort(); // pour un travailleur, les mesures sont dans l'ordre de measureArray).sort(); // le tri est alphab�tique.
    this.measureCountByWorker = this.workerIds.map(function (id) { return this._measureByWorker[id].length; }, this); // l'ordre sera celui des id tri�s alphab�tiquement
...
zygotine.M.WorkerDigest.prototype = {
    ...
    updateMuValues: function (muOverall, muWorker) {
        ...
        this.muWorker = muWorker.slice(0); // on cr�e une copie du tableau.
    },
    ...
    getAverages: function () {
        ...
        for (let i = 0; i < this.workerIds.length; i++) {
            ...
            let nMeasures = this._measureByWorker[workerId].length; // nombre de mesures associ�es � un travailleur (workerId)
    ...
    getInits: function () {
        ...
        var wId;
        var sum = 0;
        var workerInfo = []; // une entr�e pour chaque travailleur
    ...
    getBeta: function (muOverall, muWorker) {
        ...
        // le programme appelant n'a pas besoin de residuals, mais fournit un moyen de se comparer � R.
