...
Object.prototype.approx_mode = function(x)
{
  // x: starting point
...
} // end of Object.approx_mode


Object.prototype.area = function(upper_bound, lower_bound) 
{
...
Object.prototype.cdf = function(x)
{
...
Object.prototype.compute_M = function(mode = this.mode())
{
...
Object.prototype.higher_local_max = function(x_local_min, hs, epsilon)
{
  var change,
      fp,
      local_mode = [];
...
Object.prototype.mode_SafeStartingPoint = function(x, logfp, logfs, debug=false)
{
  var cont = true;
  var iter = 0;
...
Object.prototype.ProtectAgainstNaN_logfp = function(x, previous_x)
{
...
Object.prototype.quantile = function(p, xstart, debug=false, epsilon=1e-8)
{
  ...
  //
  // See top of Object.prototype.rgen for more info.
  ...
  var rank = p_array.rank();
  
  var p_split = p_array.split(u_xstart);
  ...
  {
    let rearranged_q = [];
    for (let i=0; i<rank.length; i++) rearranged_q.push(q[rank[i]]);
    return {error: false, x: rearranged_q};
  }
  else return {error: false, x: q[0]};
} // end of Object.quantile 


Object.prototype.QuickLookAtBorderAsARemoteEndpoint = function(target, remote_start, left_side=true)
{
  var cont = true,  
...
Object.prototype.ref_points = function(xstart, remote_start, debug_code=0, f_ratio_remote=1e-8, epsilon=1e-6) 
{  
  ...
  if (cont)
  {
    ...
    { 
      ...
      console.log("\tlogf=%f\n\ttarget=%f", this.logf(x).round(3), target.round(3));
    }
  }
  ...
  out.error = false;
  return out;
} // end of Object.ref_points


Object.prototype.remoteRef_SafeStartingPoint = function(x_start, target, left_side, debug=false, epsilon=1e-6)
{
  ...
  while (cont)
  {
    ...
    if (debug) console.log("trying x = %f (logf=%f, logfp=%f)", x, logf.round(3), logfp.round(3));
...
Object.prototype.rgen = function(U, xstart, epsilon=1e-8)
{
  // The object funs [this] includes:
  //   - The functions:
  //       logf, logf_prime, logf_second
...
Object.prototype.rgen_icdf = function(U, xstart, remote_start, debug_code, epsilon=1e-8)
{
